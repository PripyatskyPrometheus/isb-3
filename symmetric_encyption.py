...
def generate_symmetric_key(length: int) -> bytes:
    """
    The function generate a symmetric key for symmetric encryption algorithm.
...
def symmetric_encrypt(key: bytes, text: bytes) -> bytes:
    ...
    try:
        ...
        logging.error(f"Symmetric encryption was not successful: {str(err)}")
        raise Exception("Symmetric encryption was not successful")
